...
public class PianoReceiver implements Receiver, MetaEventListener {
	...
	private static final String[] sm_astrKeyNames = { "C", "C#", "D", "D#", "E", "F", "F#", "G",
			"G#", "A", "A#", "B" };

	private static final String[] sm_astrKeySignatures = { "Cb", "Gb", "Db", "Ab", "Eb", "Bb", "F",
			"C", "G", "D", "A", "E", "B", "F#", "C#" };
	private static final String[] SYSTEM_MESSAGE_TEXT = {
			"System Exclusive (should not be in ShortMessage!)", "MTC Quarter Frame: ",
			"Song Position: ", "Song Select: ", "Undefined", "Undefined", "Tune Request",
	...
	private static final String[] QUARTER_FRAME_MESSAGE_TEXT = { "frame count LS: ",
			"frame count MS: ", "seconds count LS: ", "seconds count MS: ", "minutes count LS: ",
	...
	private MIDIPanel mv;
	
	public PianoReceiver(PrintStream printStream, MIDIPanel mv) {
		this(printStream, mv, false);
	}

	public PianoReceiver(PrintStream printStream, MIDIPanel mv, boolean bPrintTimeStampAsTicks) {
	...
	public void send(MidiMessage message, long lTimeStamp) {
		lTimeStamp = this.mv.getSequencer().getTickPosition();
		long max = this.mv.getSequencer().getTickLength();
		...
		if (m_bPrintTimeStampAsTicks) {
			strTimeStamp = "tick " + lTimeStamp + "/" + max + ": ";
		} else {
			if (lTimeStamp == -1L) {
...
//		System.out.println(message.getClass().toString());
	...
	public String decodeMessage(ShortMessage message) {
		...
		if (message.getCommand() != 0xF0) {
			...
			String strChannel = String.format("channel %02d: ", nChannel);
